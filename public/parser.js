...
function parse(gltf, { fileName = "model", ...options } = {}) {
  ...
  const hasInstances =
    ...
    Object.keys(duplicates.geometries).length > 0;
  ...
  function handleProps(obj) {
    ...
    if (!instanced) {
      ...
      if (obj.instanceMatrix)
        result += `instanceMatrix={${node}.instanceMatrix} `;
    ...
    if (
      ...
      if (
        ...
        if (
          ...
          children = "";
          if (first.children)
            first.children.forEach((child) => (children += print(child, true)));
          return children;
        }
      }
  ...
  function print(obj, silent = false) {
    ...
    // Collect children
    if (obj.children)
      obj.children.forEach((child) => (children += print(child)));
